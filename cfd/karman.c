...
int coord[2], id;
int unitx, unity, remainx, remainy;
...
int main(int argc, char *argv[])
{
    ...
    float **local_p = alloc_floatmatrix(imax+2, jmax+2);

    if (!u || !v || !f || !g || !p || !rhs || !flag|| !local_p) {
    ...
    if(par_flag == 1)
    {
        ...
        cartsian_topology(imax, jmax);
        do_decomposition(imax, jmax);
    }
    ...
    for (t = 0.0; t < t_end; t += del_t, iters++) {
        ...
        if (ifluid > 0) {
            if(par_flag == 1){
                 itersor = par_poisson(local_p, p, rhs, flag, imax, jmax, delx, dely,
